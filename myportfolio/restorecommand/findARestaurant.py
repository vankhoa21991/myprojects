...
def findAllRestaurant():
    latitude, longitude = getCurrentPosition()
    url = ('https://api.foursquare.com/v2/venues/search?client_id=%s&client_secret=%s&v=20130815&ll=%s,%s' % (foursquare_client_id, foursquare_client_secret,latitude,longitude))
    h = httplib2.Http()
    result = json.loads(h.request(url,'GET')[1])
    RestaurantsInfo = []
    if result['response']['venues']:
        ...
        #Grab the first image
            ...
            restaurantInfo = {'id':venue_id, 'name':restaurant_name, 'address':restaurant_address, 'image':imageURL}
            RestaurantsInfo.append(restaurantInfo)
